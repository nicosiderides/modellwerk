import {groupBarPatterns,groupPlatePatterns,patternPieces,patternProgress,barNumbers,plateNumbers} from './bar-patterns.js';
import {demoProject,validateProject,optimize,metrics,signature,PALETTE,MAX_PIECES,MAX_PIECE_QTY} from './engine.js';
...
function overview(){
 const m=currentMaterial(),group=m?groupFor(m):null,bins=group?.bins||[],patterns=groupBarPatterns(bins);
 return `${metricCards()}<div class="workspace-grid" style="grid-template-columns:1fr"><section class="panel"><div class="panel-head"><div><h2>Distribución de corte</h2><p>Cada milímetro se aprovecha.</p></div><span class="tag ${project.plan?'':'orange'}">${project.plan?'Plan calculado':'Pendiente'}</span></div><div class="material-selector">${icon(m?.kind==='plate'?'layers':'bar')}<select aria-label="Material del plano" id="preview-material">${project.materials.map(v=>`<option value="${esc(v.id)}" ${v.id===m?.id?'selected':''}>${esc(v.name)} · ${esc(v.spec)}</option>`).join('')}</select><span class="tag">${fmt(m?.length||0)} mm</span></div><div class="cut-preview">${bins.length?(m.kind==='tube'?`<div class="ruler">${[0,.25,.5,.75,1].map(k=>`<span>${fmt(k*m.length)}</span>`).join('')}</div>${patterns.slice(0,4).map(pattern=>`<div class="pattern-preview">${barPatternDiagram(pattern,m)}<span class="pattern-repeat">× ${pattern.bins.length}</span></div>`).join('')}${patterns.length>4?`<div class="muted" style="font-size:11px;padding-left:44px;margin-top:14px">+ ${patterns.length-4} patrones en el plan completo</div>`:''}`:plateDiagram(bins[0],m)):empty(project.plan?'Sin piezas para este material':'Listo para calcular',project.plan?'Seleccioná otro material o agregá piezas.':'Cargá tus piezas y presioná Optimizar corte.')}</div><div class="cut-footer"><span class="legend-item"><i class="swatch" style="background:#de794e"></i>Pieza de corte</span><span class="legend-item"><i class="swatch" style="background:#dce4d3"></i>Remanente</span><span>Disco: ${project.settings.kerf} mm</span>${btn('Ver planos','goto-plans','ghost small','arrow')}</div></section></div>${pieceTable()}`;
}
...
function patternParts(pattern){
 return `<div class="table-wrap"><table><thead><tr><th>Pieza</th><th>Largo (mm)</th><th class="text-right">Cantidad total</th></tr></thead><tbody>${patternPieces(pattern).map(p=>`<tr><td>${esc(p.id)} · ${esc(p.name)}</td><td>${fmt(p.length,3)}</td><td class="text-right">${p.count}</td></tr>`).join('')}</tbody></table></div>`;
}

function barPatternCard(pattern,m){
 const b=pattern.representative,status=patternProgress(pattern,project.completed),done=status.completed.length===status.total;
 const next=status.available[0],last=status.completed.at(-1),quantity=pattern.bins.reduce((n,bin)=>n+bin.parts.length,0),visibleBins=pattern.bins.slice(0,BAR_DETAIL_LIMIT);
 const hidden=status.total-visibleBins.length;
 return `<section class="panel plan-card ${done?'done':''}" data-pattern="${esc(pattern.id)}"><div class="panel-head"><div><h2>Patrón ${String(pattern.index).padStart(2,'0')} <span class="tag ${done?'':status.completed.length?'blue':'orange'}">${done?'Completado':status.completed.length?'En proceso':'Por cortar'}</span></h2><p>${esc(m.name)} · ${esc(m.spec)} · ${fmt(m.length)} mm</p></div><span class="pattern-quantity">× ${status.total} ${status.total===1?'barra':'barras'}</span></div><div class="plan-body"><div class="pattern-heading"><strong>Repetir este corte ${status.total} ${status.total===1?'vez':'veces'}</strong><span>${fmt(b.utilization,1)} % útil por barra</span></div>${barPatternDiagram(pattern,m,true)}<p class="details-caption">${b.parts.length} piezas por barra · ${quantity} piezas en total · ${fmt(b.remaining,3)} mm de remanente por barra</p><details class="details-caption" data-details="${esc(pattern.id)}-pieces"><summary>Piezas del patrón · ${quantity} piezas en total</summary>${patternParts(pattern)}</details><details class="details-caption" data-details="${esc(pattern.id)}-bars"><summary>Control por barra · ${barNumbers(pattern.bins)}</summary><div class="pattern-bars">${visibleBins.map(bin=>{const cut=project.completed.includes(bin.id);return `<label class="pattern-bar"><input type="checkbox" data-bin="${esc(bin.id)}" ${cut?'checked':''}><span><strong>B${String(bin.index).padStart(2,'0')}</strong> · ${cut?'Cortada':'Pendiente'}<small>${[...new Set(bin.parts.map(p=>p.id))].map(esc).join(' · ')}</small></span></label>`;}).join('')}</div>${hidden?`<p class="details-caption">Se muestran las primeras ${BAR_DETAIL_LIMIT} barras de este patrón. Usá “Registrar 1 barra” para continuar el avance de las ${fmt(hidden)} restantes sin sobrecargar la pantalla.</p>`:''}</details></div><div class="plan-tools"><span><strong>${status.completed.length} de ${status.total}</strong> barras cortadas</span><div class="actions">${last?btn('Deshacer 1','complete-bin','small ghost','refresh',`data-id="${esc(last.id)}"`):''}${btn('Registrar 1 barra','complete-bin','small dark','check',`data-id="${esc(next?.id||'')}" ${next?'':'disabled'}`)}</div></div></section>`;
}
...
function platePatternParts(pattern){
 return `<div class="table-wrap"><table><thead><tr><th>Pieza</th><th>Medida (mm)</th><th class="text-right">Cantidad total</th></tr></thead><tbody>${patternPieces(pattern).map(p=>`<tr><td>${esc(p.id)} · ${esc(p.name)}</td><td>${fmt(p.length,3)} × ${fmt(p.width,3)}</td><td class="text-right">${p.count}</td></tr>`).join('')}</tbody></table></div>`;
}

function platePatternCard(pattern,m){
 const b=pattern.representative,status=patternProgress(pattern,project.completed),done=status.completed.length===status.total;
 const next=status.available[0],last=status.completed.at(-1),quantity=pattern.bins.reduce((n,bin)=>n+bin.parts.length,0),visibleBins=pattern.bins.slice(0,BAR_DETAIL_LIMIT),hidden=status.total-visibleBins.length;
 return `<section class="panel plan-card ${done?'done':''}" data-pattern="${esc(pattern.id)}"><div class="panel-head"><div><h2>Patrón ${String(pattern.index).padStart(2,'0')} <span class="tag ${done?'':status.completed.length?'blue':'orange'}">${done?'Completado':status.completed.length?'En proceso':'Por cortar'}</span></h2><p>${esc(m.name)} · ${esc(m.spec)} · ${fmt(m.length)} × ${fmt(m.width)} mm</p></div><span class="pattern-quantity">× ${status.total} ${status.total===1?'placa':'placas'}</span></div><div class="plan-body"><div class="pattern-heading"><strong>Repetir este corte ${status.total} ${status.total===1?'vez':'veces'}</strong><span>${fmt(b.utilization,1)} % útil por placa</span></div>${platePatternDiagram(pattern,m)}<p class="details-caption">${b.parts.length} piezas por placa · ${quantity} piezas en total · ${b.free.length} regiones de remanente por placa</p><details class="details-caption" data-details="${esc(pattern.id)}-pieces"><summary>Piezas del patrón · ${quantity} piezas en total</summary>${platePatternParts(pattern)}</details><details class="details-caption" data-details="${esc(pattern.id)}-sequence"><summary>Ver secuencia de guillotina y coordenadas</summary><div class="table-wrap">${sequenceTable(b,true)}</div></details><details class="details-caption" data-details="${esc(pattern.id)}-plates"><summary>Control por placa · ${plateNumbers(pattern.bins)}</summary><div class="pattern-bars">${visibleBins.map(bin=>{const cut=project.completed.includes(bin.id);return `<label class="pattern-bar"><input type="checkbox" data-bin="${esc(bin.id)}" ${cut?'checked':''}><span><strong>PL${String(bin.index).padStart(2,'0')}</strong> · ${cut?'Cortada':'Pendiente'}<small>${[...new Set(bin.parts.map(p=>p.id))].map(esc).join(' · ')}</small></span></label>`;}).join('')}</div>${hidden?`<p class="details-caption">Se muestran las primeras ${BAR_DETAIL_LIMIT} placas de este patrón. Usá “Registrar 1 placa” para continuar el avance de las ${fmt(hidden)} restantes sin sobrecargar la pantalla.</p>`:''}</details></div><div class="plan-tools"><span><strong>${status.completed.length} de ${status.total}</strong> placas cortadas</span><div class="actions">${last?btn('Deshacer 1','complete-bin','small ghost','refresh',`data-id="${esc(last.id)}"`):''}${btn('Registrar 1 placa','complete-bin','small dark','check',`data-id="${esc(next?.id||'')}" ${next?'':'disabled'}`)}</div></div></section>`;
}

function printBarPattern(pattern,m){
 const b=pattern.representative,status=patternProgress(pattern,project.completed);
 return `<section class="print-section"><h2>Patrón ${String(pattern.index).padStart(2,'0')} · ${esc(m.name)} · ${esc(m.spec)}</h2><p><strong>REPETIR ESTE CORTE ${status.total} ${status.total===1?'VEZ':'VECES'} · COMPRAR ${status.total} ${status.total===1?'BARRA':'BARRAS'} DE ${fmt(m.length)} mm</strong></p><p>Barras: ${barNumbers(pattern.bins)} · ${status.completed.length} cortadas · ${status.total-status.completed.length} pendientes</p>${barPatternDiagram(pattern,m)}<p>${b.parts.length} piezas por barra · ${b.parts.length*status.total} piezas en total · ${fmt(b.utilization,1)} % útil por barra</p><p><strong>Secuencia por barra:</strong> ${b.parts.map((p,i)=>`${i+1}. ${fmt(p.length,3)} mm`).join(' → ')}</p>${patternParts(pattern)}<p>Remanente por barra: ${fmt(b.remaining,3)} mm. Identificar cada uno como R-${esc(m.id)}-[número de barra]-1.</p><p>Control de repeticiones: ______ / ${status.total} &nbsp; Responsable: __________________</p></section>`;
}

function printPlatePattern(pattern,m){
 const b=pattern.representative,status=patternProgress(pattern,project.completed),quantity=pattern.bins.reduce((n,bin)=>n+bin.parts.length,0);
 return `<section class="print-section"><h2>Patrón ${String(pattern.index).padStart(2,'0')} · ${esc(m.name)} · ${esc(m.spec)}</h2><p><strong>REPETIR ESTE CORTE ${status.total} ${status.total===1?'VEZ':'VECES'} · COMPRAR ${status.total} ${status.total===1?'PLACA':'PLACAS'} DE ${fmt(m.length)} × ${fmt(m.width)} mm</strong></p><p>Placas: ${plateNumbers(pattern.bins)} · ${status.completed.length} cortadas · ${status.total-status.completed.length} pendientes</p>${platePatternDiagram(pattern,m)}<p>${b.parts.length} piezas por placa · ${quantity} piezas en total · ${fmt(b.utilization,1)} % útil por placa</p>${platePatternParts(pattern)}<p><strong>Secuencia de guillotina por placa</strong></p>${sequenceTable(b,true)}<p>Control de repeticiones: ______ / ${status.total} &nbsp; Responsable: __________________</p></section>`;
}
...
function sequenceTable(b,generic=false){return `<table><thead><tr><th>Paso</th><th>Origen X / Y (mm)</th><th>Región (mm)</th><th>Separación local</th></tr></thead><tbody>${b.parts.map((p,i)=>`<tr><td>${i+1} · ${generic?'Corte '+(i+1):esc(p.id)}</td><td>${fmt(p.x,1)} / ${fmt(p.y,1)}</td><td>${fmt(p.region.w,1)} × ${fmt(p.region.h,1)}</td><td>${p.split==='horizontal'?`Horizontal a ${fmt(p.h,1)}, luego vertical a ${fmt(p.w,1)}`:`Vertical a ${fmt(p.w,1)}, luego horizontal a ${fmt(p.h,1)}`}</td></tr>`).join('')}</tbody></table>`;}
...
function documentsView(){
 ...
 ['cut','Orden y planos de corte','Distribución, medidas y secuencia de corte de cada pieza. Listos para imprimir o guardar como PDF.','Imprimir / PDF','print-plan',!project.plan],
...
function printDocument(type){
 if((type==='plan'||type==='stock')&&!project.plan){toast('Primero calculá un plan de corte.');return;}
 const header=`<div class="print-header"><div><h1>MODELLWERK / CUT</h1><p>${esc(project.name)} · ${esc(project.reference)}</p></div><div>${type==='plan'?'ORDEN DE CORTE':'MATERIALES Y COMPRA'}<p>${new Date().toLocaleDateString('es-AR')}</p></div></div>`;
 let content='';
 if(type==='plan'){
  content=`<p>Medidas en mm · Corte: ${project.settings.kerf} · Despunte por borde: ${project.settings.trim} · Giro de placa: ${project.settings.rotate?'sí':'no'}</p><p>Plan heurístico de cortes rectos. Verificar medidas, kerf y sujeción antes de fabricar.</p>${project.plan.rejected.length?`<p><strong>ATENCIÓN: ${project.plan.rejected.length} piezas fuera de formato NO incluidas en los planos: ${[...new Set(project.plan.rejected.map(p=>p.id))].map(esc).join(', ')}.</strong></p>`:''}`;
